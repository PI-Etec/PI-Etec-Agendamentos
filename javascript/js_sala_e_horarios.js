// Script de seleção de sala e horários
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateResumo = () => {
    ...
    resumoEl.textContent = parts.length
      ? parts.join(" | ")
      : "Selecione sala, data e horário";
  };
  ...
  function render() {
    ...
    const weekHeader = weekdays
      .map((w) => `<div><a href="#" tabindex="-1">${w}</a></div>`)
      .join("");
    ...
    calendarRoot
      .querySelectorAll(
        '.calendar-body a[data-type="current"]:not(.is-disabled)',
      )
      .forEach((a) => {
  ...
  const timeButtons = document.querySelectorAll(
    ".lista-horarios .list-group-item",
  );
    ...
    btnConfirmar.addEventListener("click", () => {
      ...
      try {
        localStorage.setItem(
          "agendamentoSalaHorario",
          JSON.stringify(agendamentoSala),
        );
        alert(
          "Sala, data e horário salvos. Agora selecione materiais/reagentes.",
        );
