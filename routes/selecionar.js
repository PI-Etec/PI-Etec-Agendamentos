...
// [GET] /selecionar/materials — Listar materiais
  ...
  try {
    ...
    if (!updated) {
      return res.status(400).json({
        success: false,
        message: "Quantidade insuficiente ou material não encontrado.",
      });
    }
    ...
    if (err instanceof mongoose.Error.CastError) {
      return res
        .status(400)
        .json({ success: false, message: "ID do material inválido." });
    }
    next(err);
  }
});
...
router.post("/kits", async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(selections) || selections.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Nenhuma seleção fornecida." });
    }
    ...
    // Aplica decrementos nos materiais; em caso de falha, faz rollback
    const applied = [];
    try {
      for (const it of items) {
        ...
        if (!updated) {
          ...
          throw {
            code: "INSUFFICIENT",
            message: `Quantidade insuficiente para o material ${it.material || it.materialId}`,
          };
        }
      ...
      try {
        for (const a of applied) {
          await Material.findByIdAndUpdate(a.materialId, {
            $inc: { quantidade: a.quantidade },
          });
        }
      } catch (rbErr) {
        console.error("Erro ao fazer rollback em selecionar/kits:", rbErr);
        // se rollback falhar, retornar erro crítico
        return res.status(500).json({
          success: false,
          message:
            "Erro crítico ao reverter alterações de estoque. Contate o administrador.",
        });
      }
      ...
      if (applyErr && applyErr.code === "INSUFFICIENT") {
        return res
          .status(400)
          .json({ success: false, message: applyErr.message });
      }
    ...
    // Decrementos aplicados com sucesso — salvar o kit (adiciona campo `type`)
    const itemsWithType = items.map((it) => ({
...
router.post("/kits/combined", async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(materials) && !Array.isArray(reagents)) {
      return res
        .status(400)
        .json({ success: false, message: "Nenhuma seleção válida fornecida." });
    }
    ...
    try {
      for (const it of items) {
        ...
        if (it.type === "material") {
          ...
          if (!updated)
            throw {
              code: "INSUFFICIENT",
              message: `Quantidade insuficiente para o reagente ${it.material || it.materialId}`,
            };
          appliedReg.push({
            materialId: it.materialId,
            quantidade: it.quantidade,
      ...
      try {
        ...
        for (const a of appliedReg) {
          await Reagente.findByIdAndUpdate(a.materialId, {
            $inc: { quantidade: a.quantidade },
          });
        }
      } catch (rbErr) {
        console.error(
          "Erro ao fazer rollback em selecionar/kits/combined:",
          rbErr,
        );
        return res.status(500).json({
          success: false,
    ...
    const kit = new SelecaoKit({
      name: kitName !== undefined ? kitName : "",
      items: saveItems,
    });
    await kit.save();

    console.log("Kit salvo em selecionar/kits/combined:", {
      kitId: kit._id,
      name: kit.name,
    });
    ...
    if (err instanceof mongoose.Error.CastError) {
      return res
        .status(400)
        .json({ success: false, message: "ID inválido na seleção." });
    }
    next(err);
  }
});

// [GET] /selecionar/kits — Listar kits salvos
        ...
        const items = Array.isArray(k.items)
          ? await Promise.all(
              k.items.map(async (it) => {
                ...
                if (currentType && currentType !== "item")
                  return { ...base, type: currentType };
                // detectar pelo materialId nas coleções
                try {
                  if (it.materialId) {
                    const mat = await Material.findById(it.materialId)
                      .lean()
                      .select("_id");
                    if (mat) return { ...base, type: "material" };
                    const reg = await Reagente.findById(it.materialId)
                      .lean()
                      .select("_id");
                  ...
                  console.error(
                    "Falha na detecção de tipo para item em selecionar/kits:",
                    e && e.message,
                  );
                }
