...
// [GET] /reagentes — Listar reagentes
...
router.post("/", async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      reagente: { id: r._id, reagente: r.reagente, quantidade: r.quantidade },
    });
  } catch (err) {
    next(err);
  }
...
router.get("/:id", async (req, res, next) => {
  try {
    const r = await Reagente.findById(
      req.params.id,
      "reagente quantidade",
    ).lean();
    if (!r)
      return res
        .status(404)
        .json({ success: false, message: "Reagente não encontrado." });
    res.json({ success: true, reagente: r });
  } catch (err) {
    if (err instanceof mongoose.Error.CastError) {
      return res
        .status(400)
        .json({ success: false, message: "ID do reagente inválido." });
    }
    next(err);
  }
});
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    if (req.body.quantidade !== undefined)
      updates.quantidade = Number(req.body.quantidade);

    const updated = await Reagente.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true,
      context: "query",
    });
  ...
  try {
    ...
    if (!updated) {
      return res.status(400).json({
        success: false,
        message: "Quantidade insuficiente ou reagente não encontrado.",
      });
    }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error("Erro ao fazer rollback em reagentes/kits:", rbErr);
